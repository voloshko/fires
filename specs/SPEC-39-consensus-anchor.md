...
Реализовано: `drop_far(pred, far_px, anchor)`, `ensemble.predict(consensus=True)`,
`inference.py --consensus`; тест «`anchor=None` даёт прежний результат» зелёный.
Замер (`scripts/exp_siam_confirm.py`, пять групповых фолдов соседа, пул по 144
чипам вне обучения и вне выбора варианта), взвешенно:

| | пул | фолды Δ к v17 |
|---|---|---|
| v17 (оптика 50 % + сиам 50 %) | 0.6987 | — |
| v17 + якорь согласия | **0.7098** | +0.0158 / 0.0000 / **−0.0055** / +0.0074 / +0.0374 |

Чистое небо 0.7325 → 0.7452. Фолд 4 (цель спеки): из четырёх потерянных пожаров
найдены три; `BS_tr_000218` — согласия нет ни в одном пикселе, фильтр бессилен.

**Почему `partial`, а не `implemented`.** Критерий приёмки «ни один фолд не
теряет больше 0.005» не выполнен: фолд 2 теряет 0.0055. Порог после результата
не двигали. Пул +0.011 — втрое больше сидового шума (0.004), поэтому по решению
пользователя фильтр вошёл в v18 (SPEC-19-LIVE-018) с этим промахом, записанным
в receipt. Не сделано: выбор порога согласия (доля пикселей, вероятность) —
измерен только бинарный argmax обеих ветвей; вариант выбран на тех же фолдах,
на которых измерен.
...
Реализовано: `drop_far(anchor=)`, `ensemble.predict(consensus=)`,
`inference.py --consensus`, тест переключения якоря. Замер на пяти групповых
фолдах (receipt SPEC-39-FOLDS-001): пул 0.6987 → 0.7098 взв., чистое небо
0.7325 → 0.7452; по фолдам +0.016 / 0 / −0.0055 / +0.007 / +0.037. Первый
критерий приёмки выполнен, второй («ни один фолд не теряет больше 0.005»)
формально нарушен на 0.0005 — порог не двигался, факт записан.

На тестовой выборке финальная семёрка сетей с флагом даёт байт-в-байт v17:
якорь согласия не сменил главное пятно ни на одном из 89 чипов. Поэтому v18
как отдельная версия **не публикуется**, сабмит остаётся v17. Флаг сохранён в
коде как страховка от режима отказа, который на этих чипах не встретился.
Не сделано: другие варианты якоря (порог по вероятности согласия).
