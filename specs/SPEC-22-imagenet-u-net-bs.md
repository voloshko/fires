...
**Отвергнуто по замеру.** Четыре кодировщика `segmentation_models_pytorch` с
весами ImageNet против сети с нуля на той же оптике (0.7240 взв., с бустингом):
MiT-B0 на оптике 0.7055, MiT-B2 на оптике 0.7032, MiT-B0 на 19 каналах 0.6717,
ResNet34 на оптике 0.6926. В ансамбле с девяткой — нейтрально (0.7264 / 0.7267
против 0.7229). ImageNet-фильтры не переносятся на индексы гари и
мультиспектральные полосы; рецепт CEMS-Wildfire на нашей выборке не
воспроизводится. Стенд `ARCH=smp:<enc>` и загрузка таких бандлов в `unet.load`
оставлены. Запись в `evidence/hypotheses.md`.
