...
**Отвергнуто по замеру, с большим запасом.** Две оптические сети с дрожанием
и нормализацией `normalise_post` (МНК post→pre по пикселям с |dNBR| < 0.05):
0.5605 / 0.5668 взв. против 0.7162 / 0.7259 без неё; mIoU_sev 0.39–0.40 против
0.66–0.71. Причина, по всей видимости, в самой идее, а не в реализации: разница
между датами и есть сигнал гари, а линейная подгонка по «устойчивым» пикселям в
чипах с большой гарью опирается на смещённое множество и частично снимает эту
разницу; дрожание, напротив, учит сеть терпеть разницу, не удаляя её. Тест
`test_normalise_post_возвращает_pre_вне_гари` зелёный — код делает то, что
задумано. Функция, флаг `radnorm` в бандле и загрузка оставлены. Проверка
реализации на реальном чипе (например, доля «устойчивых» пикселей по чипам)
не проводилась: масштаб проигрыша делает её неинформативной для решения.
