...
**Отложено по условию самой спеки.** Она выполняется после SPEC-22 и SPEC-23 и
закрывается `deferred` без прогона, если оба проигрывают сети с нуля. Оба
проиграли: ImageNet-кодировщики 0.67–0.71 против 0.724 (SPEC-22), Prithvi-EO-2.0
0.60–0.62 в трёх режимах (SPEC-23). Третий предобученный кодировщик той же
природы — на других сенсорах, не на гарях — информации не добавит; GPU-час
дороже. Возобновить, если появится набор с разметкой степеней поражения, на
котором DOFA дообучали до нас.
