...
def predict(net_model, boost_model, chip: BsChip,
    ...
    # Под маской «гарь/фон» решает сеть одна, а СТЕПЕНЬ внутри найденной гари —
    # обычная смесь: бустинг различает степени и по отражениям через облако
    # (154 тыс. верных из 192 тыс. против 150 тыс. у сети; +0.0025 взв. на
    # обеих половинах настроечных чипов, плато веса 0.0–0.6).
    blind = ~chip.valid()
    burn_blind = p_net.argmax(2) > 0
    sev_mixed = (mixed[..., 1:].argmax(2) + 1).astype(np.uint8)
    pred[blind] = np.where(burn_blind, sev_mixed, 0)[blind]
