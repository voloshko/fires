"""SPEC-47: SWIR-индексы как признаки — бустинг и (когда есть) оптика, парно по фолдам, рецепт v20."""
...
POOL, FOLDS, LOST, CLR, BOOST_ALONE = {}, {}, {}, {}, {}
for f in range(5):
    bs = OWN / f'bs-confirm-boost-f{f}-swir-v1'
    if not (bs / 'probabilities.npy').exists(): continue
    ...
    PB, PO = (np.load(base[k] / 'probabilities.npy').astype(np.float32) for k in ('boost', 'optical')); PBS = np.load(bs / 'probabilities.npy').astype(np.float32)
    SV = np.load(OWN / f'bs-confirm-siam-f{f}-over-v1/probabilities.npy').astype(np.float32)
    oo = OWN / f'bs-confirm-optical-f{f}-swir-v1'; POS = np.load(oo / 'probabilities.npy').astype(np.float32) if (oo / 'probabilities.npy').exists() else None
    for name, pb in (('бустинг база', PB), ('бустинг SWIR', PBS)):
        out = np.where(pb.argmax(3) > 0, pb[..., 1:].argmax(3) + 1, 0); e = BOOST_ALONE.setdefault(name, ([], [])); e[0].append(T); e[1].append(out)
    combos = [('v20', PB, PO, SV), ('v20 + бустинг SWIR', PBS, PO, SV)]
    if POS is not None: combos += [('v20 + оптика SWIR', PB, POS, SV), ('v20 + бустинг SWIR + оптика SWIR', PBS, POS, SV)]
    for name, pb, po, ps in combos:
...
for name, e in BOOST_ALONE.items(): r = pooled(e); print(f'{name:36s} один: {r["iou_burn"]:.4f}/{r["miou_sev"]:.4f} взв {w(r):.4f}')
base = np.array(FOLDS['v20'])
for name, v in FOLDS.items():
    r = pooled(POOL[name]); print(f'{name:36s} пул {len(v)} ф: {r["iou_burn"]:.4f}/{r["miou_sev"]:.4f} взв {w(r):.4f} | чистое небо {CLR[name][0]/CLR[name][1]:.4f} | Δ к v20 {np.round(np.array(v)-base,4).tolist()} | потеряно {LOST[name]}')
