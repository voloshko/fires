"""SPEC-41: выцветший сиамский сид против обычного, парно по фолдам, в рецепте v19."""
...
POOL, FOLDS, LOST = {}, {}, {}
for f in range(5):
    fade = OWN / f'bs-confirm-siam-f{f}-fade-v1'
    if not (fade / 'probabilities.npy').exists(): continue
    ...
    PF = np.load(fade / 'probabilities.npy').astype(np.float32)
    for name, PS in (('v19: сид1+сид2', 0.5*(PS1+PS2)), ('сид2 один (парная база)', PS2), ('выцветший один', PF), ('сид1+выцветший', 0.5*(PS1+PF)), ('сид1+сид2+выцветший', (PS1+PS2+PF)/3)):
