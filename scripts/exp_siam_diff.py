...
for f in range(5):
    ...
    for tag, name in (('diff', 'сиам diff-фьюжн (SPEC-51.1)'), ('2st', 'сиам двухэтапная потеря (SPEC-51.2)'), ('soft', 'сиам мягкие метки у кромки (SPEC-52)'), ('sar', 'сиам + радар под маской (SPEC-57 = v22)'), ('nin', 'v22 + индексы без нормализации (61a)'), ('blob', 'v22 + blob loss (61b)'), ('crop', 'v22 + вырезки вокруг пятен (61c)')):
    ...
    for name, ps in variants:
        ...
        t = T > 0; p = out > 0; c = CLR.setdefault(name, [0, 0, 0, 0]); c[0] += (t & p & OK).sum(); c[1] += ((t | p) & OK).sum(); c[2] += (t & p & ~OK).sum(); c[3] += ((t | p) & ~OK).sum()
...
for name, v in FOLDS.items():
    ...
    print(f'{name:40s} сеть одна {w(ra):.4f} | рецепт пул {n} ф: {r["iou_burn"]:.4f}/{r["miou_sev"]:.4f} взв {w(r):.4f} | чистое небо {CLR[name][0]/CLR[name][1]:.4f} | под маской {CLR[name][2]/max(CLR[name][3],1):.4f} | Δ {np.round(np.array(v)-base[:n],4).tolist()} | потеряно {LOST[name]}')
