"""SPEC-48/49: оптика с картой бустинга (bch) и с локальным контрастом (lz) против базы, рецепт v21."""
...
for f in range(5):
    ...
    variants = [('v21: оптика база', PO)]
    for tag, name in (('bch', 'оптика + карта бустинга (SPEC-48)'), ('lz', 'оптика + локальный контраст (SPEC-49)')):
        q = OWN / f'bs-confirm-optical-f{f}-{tag}-v1/probabilities.npy'
        if q.exists(): variants.append((name, np.load(q).astype(np.float32)))
    for name, po in variants:
        ao = np.where(po.argmax(3) > 0, po[..., 1:].argmax(3) + 1, 0); e = ALONE.setdefault(name, ([], [])); e[0].append(T); e[1].append(ao)
        out, OK = run(PB, po, SV, chips); FOLDS.setdefault(name, []).append(w(score_bs_micro(list(T), list(out))))
...
base = np.array(FOLDS['v21: оптика база'])
