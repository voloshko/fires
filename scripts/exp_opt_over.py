"""SPEC-44: передискретизованная оптика против обычной, парно по фолдам, в рецепте v20 (сиам передискр.)."""
...
POOL, FOLDS, LOST, CLR = {}, {}, {}, {}
for f in range(5):
    oo = OWN / f'bs-confirm-optical-f{f}-over-v1'
    if not (oo / 'probabilities.npy').exists(): continue
    ...
    PSO = np.load(OWN / f'bs-confirm-siam-f{f}-over-v1/probabilities.npy').astype(np.float32)
    POO = np.load(oo / 'probabilities.npy').astype(np.float32)
    for name, po, ps in (('v19: опт + сид1+сид2', PO, 0.5*(PS1+PS2)), ('v20: опт + сиам передискр.', PO, PSO), ('опт передискр. + сиам передискр. (v21?)', POO, PSO), ('опт передискр. + сид1+сид2', POO, 0.5*(PS1+PS2))):
        ...
        for tt, o in zip(T, out):
            iou = ((tt > 0) & (o > 0)).sum() / max(((tt > 0) | (o > 0)).sum(), 1); LOST[name] = LOST.get(name, 0) + (iou < 0.3)
base = np.array(FOLDS['v20: опт + сиам передискр.'])
for name, v in FOLDS.items():
    ...
    print(f'{name:40s} пул {len(v)} ф: {r["iou_burn"]:.4f}/{r["miou_sev"]:.4f} взв {w(r):.4f} | чистое небо {CLR[name][0]/CLR[name][1]:.4f} | Δ к v20 {np.round(np.array(v)-base,4).tolist()} | потеряно {LOST[name]}')
