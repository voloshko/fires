...
def run(args):
    ...
    for epoch in range(args.epochs):
        ...
        for k in range(0,len(order)-args.batch+1,args.batch):
            ...
            if getattr(args,'gain',0): x=x*torch.as_tensor(rng.uniform(1-args.gain,1+args.gain,(x.shape[0],x.shape[1],1,1)),dtype=x.dtype,device=x.device)  # SPEC-70: усиление каналов
    ...
    bundle=dict(state=net.state_dict(),variant=args.variant,width=args.width,depth=args.depth,fusion_norm=args.variant=='siam',fusion=getattr(args,'fusion','full'),two_stage=getattr(args,'two_stage',False),soft_edge=getattr(args,'soft_edge',0),in_channels=int(X.shape[1]),sar_gate=os.environ.get('SAR_GATE')=='1',no_index_norm=getattr(args,'no_index_norm',False),blob_loss=getattr(args,'blob_loss',0),faint_crops=getattr(args,'faint_crops',0),gain=getattr(args,'gain',0),rot90=int(os.environ.get('ROT90',0)),precision=args.precision,mean=mean,std=std,seed=args.seed,epochs=args.epochs,fit=fit)
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('task',choices=['boost','train']); p.add_argument('--data',default='data/comp/train/bs'); p.add_argument('--split',default='data/comp/split_bs.json'); p.add_argument('--out',required=True); p.add_argument('--variant',choices=['optical','raw','siam'],default='optical'); p.add_argument('--seed',type=int,default=20260918); p.add_argument('--epochs',type=int,default=200); p.add_argument('--width',type=int,default=32); p.add_argument('--depth',type=int,default=7); p.add_argument('--batch',type=int,default=8); p.add_argument('--boost',default='research/bs-boost-v1'); p.add_argument('--min-extra',type=int,default=4); p.add_argument('--fold',type=int); p.add_argument('--extra'); p.add_argument('--oversample-faint',nargs=2,type=float,help='SPEC-43: порог dNBR и кратность показа бледных чипов'); p.add_argument('--no-index-norm',action='store_true',help='SPEC-61: индексы (каналы 18..27 сиама) без нормализации по чипу'); p.add_argument('--gain',type=float,default=0.0,help='SPEC-70: усиление каналов ×[1-g,1+g] в нормализованном пространстве'); p.add_argument('--blob-loss',type=float,default=0.0,help='SPEC-61: вес потери по компонентам'); p.add_argument('--faint-crops',type=int,default=0,help='SPEC-61: размер вырезок вокруг бледных пятен (0 — выкл.)'); p.add_argument('--soft-edge',type=int,default=0,help='SPEC-52: окно мягких меток у кромки (0 — выкл.)'); p.add_argument('--fusion',choices=['full','diff'],default='full',help='SPEC-51: фьюжн сиама'); p.add_argument('--two-stage',action='store_true',help='SPEC-51: двухэтапная потеря'); p.add_argument('--fade',nargs=2,type=float,help='SPEC-41: диапазон α выцветания сцены после'); p.add_argument('--encoder'); p.add_argument('--precision',choices=['fp16','bf16','fp32'],default='fp16'); p.add_argument('--debug-numerics',action='store_true'); p.add_argument('--smoke',action='store_true'); p.add_argument('--final',action='store_true',help='обучение на всех чипах без замера'); args=p.parse_args()
