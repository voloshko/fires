...
VARS = ['v22', 'R1 вместо оптики', 'R2 третий поровну', 'R3 третий 0.25']; F = {v: [] for v in VARS}; T_all, O_all, L, ALONE, E = [], {v: [] for v in VARS}, {v: 0 for v in VARS}, [], {'оптика': [], 'сиам': [], 'бустинг': [], 'prithvi': []}; OKs = []
for f in range(5):
    ...
    T_all.append(T); OKs.append(OK); ALONE.append(np.where(PR.argmax(3) > 0, PR[..., 1:].argmax(3) + 1, 0))
...
base = np.array(F['v22'])
for v in VARS:
    r = score_bs(Tc, np.concatenate([o.reshape(-1) for o in O_all[v]])); dv = np.array(F[v]) - base
    sel = dv[:3].mean() if n >= 3 else float('nan'); chk = dv[3:5].mean() if n == 5 else float('nan')
    print(f'{v:20s} пул {w(r):.4f} ({w(r)-w(score_bs(Tc, np.concatenate([o.reshape(-1) for o in O_all["v22"]]))):+.4f}) | Δ по фолдам {np.round(dv, 4).tolist()} | выбор ф0–2 {sel:+.4f}, проверка ф3–4 {chk:+.4f} | потеряно {L[v]}')
