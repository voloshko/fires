...
def hard(ids, sel=None, extra=False):
    x, y = build(ids, 20260918, sel, None, extra); b = train(x, y, 20260918)
    x, y = build(ids, 20260918, sel, b, extra); return train(x, y, 20260918)
...
t0 = time.time()
for name, mode in (('общая модель', 'one'), ('общая + признак ночи', 'flag'), ('день и ночь раздельно', 'split')):
